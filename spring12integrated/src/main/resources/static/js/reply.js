...
$(function(){
			...
			success:function(response){//response == List<ReplyDto>
				...
				for(var i=0; i < response.length; i++) {
					...
					if(memberId == response[i].replyWriter){
						var editButton = $("<i>").addClass("fa-solid fa-edit ms-10").click(editReply);
	...
	function editReply(){
		// this == 수정버튼(data-reply-no 라는 이름으로 댓글번호가 존재)
		var replyNo = $(this).data("reply-no");
		// 계획
		// - 입력창을 등록버튼, 취소버튼을 생성
		// - 등록버튼을 누르면 비동기통신으로 댓글을 수정 후 목록 갱신
		// - 취소버튼을 누르면 생성한 태그를 삭제
		var textarea = $("<textarea>").addClass("form-input w-100").attr("placeholder","변경내용 작성");
		var confirmButton = $("<button>").addClass("form-btn positive ms-10").text("수정")
										.click(function(){
		...
		var cancelButton = $("<button>").addClass("form-btn neutral").text("취소")
										.click(function(){
		...
		div.append(textarea).append(confirmButton).append(cancelButton);
